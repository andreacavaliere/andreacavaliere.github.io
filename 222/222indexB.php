...
display_table($artistUserName, $image_info_table, $dbname, $firstName, $lastName, $imageName);
...
function display_table($artistUserName, $image_info_table, $dbname, $firstName, $lastName)
{
	...
	if ($image_info_table)
	{
		...
		for ($count = 0; $count < $num_images; $count++)
		{
			...
			echo "<TD><img id='$id_name' src='https://$domain/$artistUserName/$thumb_name' onmouseover=" . '"' . "
						showDetailedView('$div_id', '$firstName', '$lastName', 'https://$domain/$artistUserName/$image_name')" . '" />';
